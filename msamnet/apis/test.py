...
from msamnet.datasets import extract_data
from msamnet.utils import get_root_logger, reduce_mean, is_main
...
def accuracy(pred_bboxes, gt_bbox, pred_masks, gt_mask, is_crowd=None, device="cuda:0"):
    ...
    ## calculating precision of 0.5~0.9 IOU threshold 
    # bbox_iou = torch.tensor([-1.], device=device)
    # det_acc_at_thrs = torch.full((5, ), -1., device=device)
    # if eval_det:
    #     gt_bbox = torch.stack(gt_bbox).to(device)
    #     bbox_iou = bbox_overlaps(gt_bbox,pred_bboxes,is_aligned=True)
    ...
    # return det_acc_at_thrs * 100., mask_iou * 100., mask_acc_at_thrs * 100.
...
def evaluate_model(epoch,
    ...
    with torch.no_grad():
        for batch, inputs in enumerate(loader):
            ...
            det_acc_list.append(batch_det_acc)
            ...
            if is_main():
                if batch % cfg.log_interval == 0 or batch == batches - 1:
                    ...
                    logger.info(f"validate - epoch [{epoch+1}]-[{batch+1}/{batches}] " +
                                ...
                                # f"DetACC@0.5: [{det_acc[0]:.2f}, {det_acc[1]:.2f}, {det_acc[2]:.2f}, {det_acc[3]:.2f}, {det_acc[4]:.2f}]" + #{det_acc:.2f}
                                f"mIoU: {mask_iou:.2f}, " +
