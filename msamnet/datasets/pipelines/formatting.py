...
class DefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        if 'img' in results:
            img = results['img']# 图像原始三通道读取
            # add default meta keys
            results = self._add_default_meta_keys(results)
            if len(img.shape) < 3:# 若a通道数小于3则在-1位置增加一维
                img = numpy.expand_dims(img, -1)
            img = numpy.ascontiguousarray(img.transpose(2, 0, 1))#一个trick， as contiguous array:连续数组，将一个内存不连续存储的数组转换为内存连续存储的数组，使得运行速度更快。
