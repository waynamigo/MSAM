...
def extract_data(inputs):
    ...
    for key, value in inputs.items():
        ...
        data = value.data
        #data shape [1, batchsize],data[0][0:64]
        '''
        {'filename': './data/images/mscoco/train2014/COCO_train2014_000000049315.jpg', 
        'expression': 'bench left', 
        'ori_shape': (466, 640, 3), 
        'img_shape': (254, 349, 3), 
        'pad_shape': (256, 352, 3), 
        'scale_factor': array([0.5453125 , 0.54506438, 0.5453125 , 0.54506438]), 
...
# tokenize方法，通过glove库替代纯天然embedding层
def tokenize(annsfile, anns_all, word_emb_cfg=None):
    ...
    if osp.exists(token2ix_path) and osp.exists(ix2token_path) and osp.exists(word_emb_path):
        ...
        return token2ix, ix2token, npz['word_emb']# npz文件的读取，保存时有参数(path,word_emb=word_emb)
    else:
        token2ix = {
            'PAD': 0,
            'UNK': 1,
