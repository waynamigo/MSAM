...
from msamnet.apis import evaluate_model, set_random_seed
from msamnet.datasets import build_dataset, build_dataloader
from msamnet.models import build_model, ExponentialMovingAverage
from msamnet.utils import (get_root_logger, load_checkpoint, init_dist, 
...
def main_worker(cfg):
    ...
    for eval_loader, _prefix in zip(dataloaders, prefix):
        if is_main():
            logger = get_root_logger()
            logger.info(f"MSAM - evaluating set {_prefix}")
            
        evaluate_model(-1, cfg, model, eval_loader)
        if cfg.ema:
            if is_main():
                logger = get_root_logger()
                logger.info(f"MSAM - evaluating set {_prefix} using ema")
...
def parse_args():
    parser = argparse.ArgumentParser(description="MSAM-test")
